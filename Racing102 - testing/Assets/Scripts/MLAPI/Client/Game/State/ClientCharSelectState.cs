...
namespace Racing102.Client
{
    ...
    /// </summary>
    [RequireComponent(typeof(CarSelectData))]
    public class ClientCharSelectState : GameStateBehaviour
    {
        ...
        public override GameState ActiveState { get { return GameState.CarSelect; } }
        public CarSelectData CarSelectData { get; private set; }

        [Header("Configuration of in-Scene Character")]
        [SerializeField]
        [Tooltip("Reference to dummy character model in the char-select screen")]
        private CarSwap m_InSceneCharacter;
        ...
        private List<UICarSelectPlayerSeat> m_PlayerSeats;
        ...
        private UICarSelectClassInfoBox m_ClassInfoBox;
        ...
        {
            Instance = this;
            CarSelectData = GetComponent<CarSelectData>();
        ...
        protected override void Start()
        {
            ...
            UpdateCharacterSelection(CarSelectData.SeatState.Inactive);
        }
        ...
        protected override void OnDestroy()
        {
            ...
            if (CarSelectData)
            {
                CarSelectData.IsLobbyClosed.OnValueChanged -= OnLobbyClosedChanged;
                CarSelectData.OnFatalLobbyError -= OnFatalLobbyError;
                CarSelectData.OnAssignedPlayerNumber -= OnAssignedPlayerNumber;
                CarSelectData.LobbyPlayers.OnListChanged -= OnLobbyPlayerStateChanged;
            }
            if (Instance == this)
                Instance = null;
        }
        ...
        /// </summary>
        private void OnLobbyPlayerStateChanged(NetworkListEvent<CarSelectData.LobbyPlayerState> lobbyArray)
        {
            UpdateSeats();

            // now let's find our local player in the list and update the character/info box appropriately
            int localPlayerIdx = -1;
            for (int i = 0; i < CarSelectData.LobbyPlayers.Count; ++i)
            {
                if (CarSelectData.LobbyPlayers[i].ClientId == NetworkManager.Singleton.LocalClientId)
                {
                    localPlayerIdx = i;
                    break;
                }
            ...
            else if (CarSelectData.LobbyPlayers[localPlayerIdx].SeatState == CarSelectData.SeatState.Inactive)
            {
            ...
            else
            {
                ...
                UpdateCharacterSelection(CarSelectData.LobbyPlayers[localPlayerIdx].SeatState, CarSelectData.LobbyPlayers[localPlayerIdx].SeatIdx);
            }
        }
        ...
        private void UpdateCharacterSelection(CarSelectData.SeatState state, int seatIdx = -1)
        {
            bool isNewSeat = m_LastSeatSelected != seatIdx;

            m_LastSeatSelected = seatIdx;
            if (state == CarSelectData.SeatState.Inactive)
            {
            ...
            else
            {
                ...
                m_InSceneCharacter.SwapToModel(CarSelectData.LobbySeatConfigurations[seatIdx].CharacterArtIdx);
                m_ClassInfoBox.ConfigureForClass(CarSelectData.LobbySeatConfigurations[seatIdx].Car);
                if (state == CarSelectData.SeatState.LockedIn && !m_HasLocalPlayerLockedIn)
                {
                    ...
                    ConfigureUIForLobbyMode(CarSelectData.IsLobbyClosed.Value ? LobbyMode.LobbyEnding : LobbyMode.SeatChosen);

                    m_HasLocalPlayerLockedIn = true;
                }
                else if (state == CarSelectData.SeatState.Active && isNewSeat)
                {
        ...
        private void UpdateSeats()
        {
            ...
            CarSelectData.LobbyPlayerState[] curSeats = new CarSelectData.LobbyPlayerState[m_PlayerSeats.Count];
            foreach (CarSelectData.LobbyPlayerState playerState in CarSelectData.LobbyPlayers)
            {
                if (playerState.SeatIdx == -1 || playerState.SeatState == CarSelectData.SeatState.Inactive)
                    continue; // this player isn't seated at all!
                if (curSeats[playerState.SeatIdx].SeatState == CarSelectData.SeatState.Inactive
                    || (curSeats[playerState.SeatIdx].SeatState == CarSelectData.SeatState.Active && curSeats[playerState.SeatIdx].LastChangeTime < playerState.LastChangeTime))
                {
        ...
        private void OnFatalLobbyError(CarSelectData.FatalLobbyError error)
        {
            switch (error)
            {
                case CarSelectData.FatalLobbyError.LobbyFull:
        ...
        {
            CarSelectData.ChangeSeatServerRpc(NetworkManager.Singleton.LocalClientId, seatIdx, false);
        }
        ...
        {
            CarSelectData.ChangeSeatServerRpc(NetworkManager.Singleton.LocalClientId, m_LastSeatSelected, true);
        }
        ...
        private void OnValidate()
        {
            if (gameObject.scene.rootCount > 1) // Hacky way for checking if this is a scene object or a prefab instance and not a prefab definition.
            {
                ...
                while (m_PlayerSeats.Count < CarSelectData.k_MaxLobbyPlayers)
                {
