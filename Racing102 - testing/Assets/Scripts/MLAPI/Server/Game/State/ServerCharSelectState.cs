...
namespace Racing102.Server
{
    ...
    public class ServerCharSelectState : GameStateBehaviour
    {
        ...
        public CarSelectData CharSelectData { get; private set; }

        private void Awake()
        {
            CharSelectData = GetComponent<CarSelectData>();
        }
        ...
        private void OnClientChangedSeat(ulong clientId, int newSeatIdx, bool lockedIn)
        {
            ...
            foreach (CarSelectData.LobbyPlayerState playerInfo in CharSelectData.LobbyPlayers)
            {
                if (playerInfo.SeatIdx == newSeatIdx && playerInfo.SeatState == CarSelectData.SeatState.LockedIn)
                {
            ...
            CharSelectData.LobbyPlayers[idx] = new CarSelectData.LobbyPlayerState(clientId,
                CharSelectData.LobbyPlayers[idx].PlayerName,
                CharSelectData.LobbyPlayers[idx].PlayerNum,
                lockedIn ? CarSelectData.SeatState.LockedIn : CarSelectData.SeatState.Active,
                newSeatIdx,
                Time.time);
        ...
        private void CloseLobbyIfReady()
        {
            foreach (CarSelectData.LobbyPlayerState playerInfo in CharSelectData.LobbyPlayers)
            {
                if (playerInfo.SeatState != CarSelectData.SeatState.LockedIn)
        ...
        private void SaveLobbyResults()
        {
            ...
            foreach (CarSelectData.LobbyPlayerState playerInfo in CharSelectData.LobbyPlayers)
            {
                lobbyResults.Choices[playerInfo.ClientId] = new LobbyResults.CarSelectChoice(playerInfo.PlayerNum,
                    CharSelectData.LobbySeatConfigurations[playerInfo.SeatIdx].Car,
        ...
        {
            for (int possiblePlayerNum = 0; possiblePlayerNum < CarSelectData.k_MaxLobbyPlayers; ++possiblePlayerNum)
            {
                bool found = false;
                foreach (CarSelectData.LobbyPlayerState playerState in CharSelectData.LobbyPlayers)
                {
        ...
        private void SeatNewPlayer(ulong clientId)
        {
            ...
            if (playerNum == -1)
            {
                ...
                CharSelectData.FatalLobbyErrorClientRpc(CarSelectData.FatalLobbyError.LobbyFull,
            ...
            CharSelectData.LobbyPlayers.Add(new CarSelectData.LobbyPlayerState(clientId, playerName, playerNum, CarSelectData.SeatState.Inactive));
        }
